...
for (const contract_ of contracts){
    const fileName = contract_.fileName;
    const contractName = contract_.contractName;

    // Read the Solidity source code from the file system
    const contractPath = path.join(__dirname + "/contracts/", fileName);
    console.log("my contract path = ", contractPath);
    ...
    const bytecodePath = path.join(__dirname + "/bin/", `${contractName}Bytecode.bin`);
